...
import com.example.demo.librairie.exception.InvoiceGenerationException;
...
public class InvoicePdfGenerator {
  ...
  private void writeCustomer(Document document, Order order) throws IOException {
    try {
      ...
      document.add(new Paragraph(customer.getFirstname() + " " + customer.getName(), NORMAL_FONT));
  ...
  private void writeLines(Document document, Order order) throws IOException {
    try {
      ...
      for (OrderLine line : order.getLines()) {
        ...
        var label =
            bookFormat.getBook().getTitle() + " (" + bookFormat.getFormat().getFormatType() + ")";
